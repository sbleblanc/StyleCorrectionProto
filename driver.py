...
def do_inference(cl: StreamingH5CorpusLoader,
                 model: TransformerS2S,
                 line: str,
                 preprocess: Callable[[str], str],
                 line_max_len: int,
                 beam_width: int,
                 decoded_max_len_scale: int,
                 noising_beta: float,
                 temperature: float,
                 device: str):
    line = line.strip()
    ...
    if line_max_len > 0 and len(line.split(' ')) > line_max_len:
        print('TOO LONG')
        return None, line
    ...
    beam_decoded = model.beam_decode_2(
        ...
        max_len=int(encoded.shape[0] * decoded_max_len_scale),
...
if config['mode'] == 'hd5_gen':
    ...
    for i in range(config['pretrain']['max_epoch']):
        ...
        for tbi, (t_enc_in, t_enc_in_key_mask, t_dec_out, t_dec_in, t_dec_in_key_mask, t_offsets) in enumerate(
            ...
            if tbi % config['eval']['interval'] == 0:
                ...
                with torch.no_grad():
                    for vbi, (
                    v_enc_in, v_enc_in_key_mask, v_dec_out, v_dec_in, v_dec_in_key_mask, v_offsets) in enumerate(
                            pds(bs=bs, which='valid')):
    ...
    for i in range(config['finetune']['max_epoch']):
        model.train()
        for tbi, (t_noised_batch, t_input_key_mask, t_eos_trunc, t_bos_trunc, t_output_key_mask) in enumerate(
                dnds(bs=bs)):
    ...
    if config['pretrain']['algo'] == 'bart':
        print('Using text infilling (BART)')
        pds_train = StreamingBARTPretrainingDataset(cl_train, tokens_per_batch=config['pretrain']['tpb'],
                                                    max_trainable_tokens=config['pretrain']['mttpb'], device=device)
        pds_valid = StreamingBARTPretrainingDataset(cl_valid, tokens_per_batch=config['pretrain']['tpb'],
                                                    max_trainable_tokens=config['pretrain']['mttpb'], device=device)
    else:
        pds_train = StreamingMASSPretrainingDataset(cl_train, tokens_per_batch=config['pretrain']['tpb'],
                                                    max_trainable_tokens=config['pretrain']['mttpb'], device=device)
        pds_valid = StreamingMASSPretrainingDataset(cl_valid, tokens_per_batch=config['pretrain']['tpb'],
                                                    max_trainable_tokens=config['pretrain']['mttpb'], device=device)
        ...
        with open(model_save_fn, 'rb') as data_file:
            ...
            if cl_train.group_indexing:
                cl_train.state = {k: loaded_data[k] for k in
                                  ['current_group', 'current_group_selector', 'current_group_offsets'] if
                                  k in loaded_data}
            else:
                cl_train.state = {k: loaded_data[k] for k in
        ...
        for tbi, (t_enc_in, t_enc_in_key_mask, t_dec_out, t_dec_in, t_dec_in_key_mask, t_offsets) in enumerate(
            ...
            if tbi % config['eval']['interval'] == 0 or max_reached:
                ...
                with torch.no_grad():
                    ...
                    if in_multigpu_mode:
                        if config['pretrain']['algo'] == 'bart':
                            out = model.module.model(v_enc_in[:1], v_dec_in[:1], v_enc_in_key_mask[:1],
                                                     v_dec_in_key_mask[:1], None)
                        else:
                            out = model.module.model(v_enc_in[:1], v_dec_in[:1], v_enc_in_key_mask[:1],
                                                     v_dec_in_key_mask[:1], v_offsets[:1])
                    else:
                        if config['pretrain']['algo'] == 'bart':
                            out = model(v_enc_in[:1], v_dec_in[:1], v_enc_in_key_mask[:1], v_dec_in_key_mask[:1], None)
                        else:
                            out = model(v_enc_in[:1], v_dec_in[:1], v_enc_in_key_mask[:1], v_dec_in_key_mask[:1],
                                        v_offsets[:1])
                    if out.numel() == 0:
                        print(v_enc_in)
                    else:
                    ...
                    train_loss_mean = torch.tensor(train_losses).mean()
                    valid_loss_mean = torch.tensor(valid_losses).mean()
                    print('{}: Sentences Processed {}/{} / Num. Steps {} : Train:{:.4f}, Valid:{:.4f}'.format(i,
                                                                                                              cl_train.current_iterating_idx -
                                                                                                              t_enc_in.shape[
                                                                                                                  0],
                                                                                                              len(
                                                                                                                  cl_train),
    ...
    #                                          device)
    # else:
    #     dnds_train, dnds_valid = get_finetune_dataset(config['finetune']['dataset']['to_use'], config,
    #                                                   cl_direct_noise_train, cl_direct_noise_valid)
    #
    # model = TransformerS2S(
    #     len(vocab),
    #     config['TransformerS2S']['emb_dim'],
    ...
    #
    # if config['finetune']['best_criteria']['use'] == 'gleu':
    #     use_gleu = True
    #     src_path = os.path.expandvars(config['finetune']['best_criteria']['gleu']['src'])
    #     refs_path = [os.path.expandvars(r) for r in config['finetune']['best_criteria']['gleu']['refs']]
    #     gleu_calculator = GLEU(config['finetune']['best_criteria']['gleu']['n'])
    #     gleu_calculator.load_sources(src_path)
    #     gleu_calculator.load_references(refs_path)
    #     if config['finetune']['best_criteria']['gleu']['preprocess']:
    #         from stylecorrection.utils.preprocess import SpacyBPEPreprocess
    #
    #         codes_fn = os.path.expandvars(config['finetune']['best_criteria']['gleu']['bpe_codes_fn'])
    #         bpe_vocab_fn = os.path.expandvars(config['finetune']['best_criteria']['gleu']['bpe_vocab_fn'])
    #         spacy_bpe_pp = SpacyBPEPreprocess(codes_fn, bpe_vocab_fn)
    #         with open(src_path, 'r') as src_f:
    #             src_sentences = [spacy_bpe_pp(line.strip()) for line in src_f]
    #     else:
    #         with open(src_path, 'r') as src_f:
    #             src_sentences = [line.strip() for line in src_f]
    #
    # train_losses = []
    # max_reached = False
    # patience_counter = 0
    ...
    #     model.train()
    #     for tbi, (
    #     t_noised_batch, t_input_key_mask, t_bos_trunc, t_eos_trunc, t_output_key_mask, t_offsets) in enumerate(
    #             dnds_train):
    ...
    #             valid_losses = []
    #             with torch.no_grad():
    #                 for vbi, (v_noised_batch, v_input_key_mask, v_bos_trunc, v_eos_trunc, v_output_key_mask,
    #                           v_offsets) in enumerate(dnds_valid):
    #                     if in_multigpu_mode:
    #                         loss = model(v_bos_trunc, v_noised_batch, v_eos_trunc, v_input_key_mask, v_output_key_mask,
    #                                      None)
    #                         loss = loss.mean()
    #                     else:
    #                         out = model(v_noised_batch, v_eos_trunc, v_input_key_mask, v_output_key_mask, None)
    ...
    #                     if vbi == config['eval']['num_valid_batch']:
    #                         break
    #                 v_noised_batch, v_input_key_mask, v_bos_trunc, v_eos_trunc, v_output_key_mask, v_offsets = next(
    #                     iter(dnds_valid))
    #                 if in_multigpu_mode:
    #                     out = model.module.model(v_noised_batch[:1], v_eos_trunc[:1], v_input_key_mask[:1],
    #                                              v_output_key_mask[:1], None)
    #                 else:
    #                     out = model(v_noised_batch[:1], v_eos_trunc[:1], v_input_key_mask[:1], v_output_key_mask[:1],
    #                                 None)
    #                 if out.numel() == 0:
    #                     print(v_noised_batch)
    #                 else:
    ...
    #                         infered_sent, original_sent = do_inference(cl_direct_noise_valid,
    #                                                                    model,
    #                                                                    sent,
    #                                                                    )
    #
    #                 train_loss_mean = torch.tensor(train_losses).mean()
    #                 valid_loss_mean = torch.tensor(valid_losses).mean()
    #                 print('{}: Sentences Processed: {}/{} / Steps {} : Train:{:.4f}, Valid:{:.4f}({:.4f})'.format(i,
    #                                                                                                               cl_direct_noise_train.current_iterating_idx -
    #                                                                                                               t_noised_batch.shape[
    #                                                                                                                   0],
    #                                                                                                               len(
    #                                                                                                                   cl_direct_noise_train),
    #                                                                                                               current_training_step,
    #                                                                                                               train_loss_mean,
    #                                                                                                               valid_loss_mean,
    #                                                                                                               best_valid_loss))
        ...
        with open(hyp_output_fn, 'w', buffering=buffering) as out_f:
            for li, line in enumerate(in_f):
                if li < config['inference']['line_offset']:
                    continue
                if config['inference']['preprocess']['activate']:
                    preprocess = spacy_bpe_pp
                else:
                    preprocess = None
                infered_line, original_line = do_inference(cl,
                                                           model,
                                                           line,
                                                           preprocess,
                                                           config['inference']['max_len'],
                                                           config['inference']['beam_width'],
                                                           config['inference']['max_len_scale'],
                                                           config['inference']['noising_beta'],
                                                           config['inference']['temperature'],
                                                           device)
                if infered_line is None:
                    print('TOO LONG')
                    continue
                else:
                    print('IN  : {}'.format(original_line))
                    if config['inference']['remove_bpe_placeholder']:
                        infered_line = infered_line.replace("@@ ", "")
                    if config['inference']['output_parallel']:
                        out_f.write('{} <split> {}\n'.format(original_line, infered_line))
                    else:
                        out_f.write('{}\n'.format(infered_line))
                    print('OUT : {}'.format(infered_line))
                # line = line.strip()
                # if config['inference']['preprocess']['activate']:
